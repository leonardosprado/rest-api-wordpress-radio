...
//API Promocao DELETE
function api_filial_delete($request){

    $slug = $request['slug'];

    $filial_id = get_filial_id_slug($slug);

    $user = wp_get_current_user();

    $author_id = (int) get_post_field('post_author', $filial_id);
    $user_id = (int) $user->ID;
    ...
    if ($user_id > 0 and ($roles === "administrator")) {
        $images = get_attached_media('image', $filial_id);
        if($images){
            foreach($images as $key => $value){
                wp_delete_attachment($value->ID, true);
            }
        }
        $response = wp_delete_post($filial_id, true);
...
function registrar_api_filial_delete(){

    register_rest_route('api', '/filial/(?P<slug>[-\w]+)', array(
        array(
            'methods' => WP_REST_Server::DELETABLE,
            'callback' => 'api_filial_delete',
        ),
    ));
}

add_action('rest_api_init','registrar_api_filial_delete');
